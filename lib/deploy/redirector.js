...
const deployToRedirector = async (config, revision) => {
  ...
  if (!configValidationResult.isValid) {
    console.error(`> Invalid redirector config: ${configValidationResult.message}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`> Error while uploading to S3: ${error.message}`);
    console.error(error);
    process.exit(1);
  }
  ...
  if (!uploadValidationResult.isValid) {
    console.error(`> S3 upload invalid:
  ...
  try {
    ...
    console.error(`> Error while updating redirector: ${error.message}`);
    console.error(error);
    process.exit(1);
  }
};

module.exports = deployToRedirector;
