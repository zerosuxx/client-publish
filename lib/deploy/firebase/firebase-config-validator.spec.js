...
describe('FirebaseConfigValidator', function() {
  ...
  describe('#validateConfig', function() {
    ...
    it('should throw error if firebase config file is missing', function() {
      config.get.withArgs('firebase.configFile').returns(false);

      const validationResult = FirebaseConfigValidator.validateConfig(config);

      expect(validationResult.isValid).to.eql(false);
      expect(validationResult.message).to.eql('FIREBASE_CONFIG_FILE is missing');
    });

    it('should throw error if firebase credentials is missing', function() {
